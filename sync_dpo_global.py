...
def sync():
    ...
    pattern = r'(<script src="data/dpo-global-bank\.js"></script>\s*<script>)(.*?)(/\* =+\s*\n   APP BOOTSTRAP)'
    m_idx = re.search(pattern, idx, re.DOTALL)
    m_404 = re.search(pattern, html404, re.DOTALL)
    if not m_idx or not m_404:
        print("WARN: could not find script block markers")
        return
    new_404 = html404[:m_404.start(2)] + m_idx.group(2) + html404[m_404.end(2):]
